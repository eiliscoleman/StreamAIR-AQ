...
parser.add_argument('-rain', '--rain', default = 'rain', dest = 'rain', help = 'Name for rainfall')
...
outpath = os.path.join(dirpath, '%s-rain-surface-level-gfs-1.0.json' % (timestr,))
gc=0
outf = open(outpath, 'w')
outf.write('{ ')
for xways in range(0, nx):
  for yways in range(0,ny):
    ...
    rawdatavals=newf.variables[args.rain][0,yways,xways]
    rounddatavals=round(rawdatavals,3)
    strval=str(rounddatavals)
    gc=gc+1
    if gc==gridsize:
      strtowrite=(' "'+strlat+', '+strlon+'": {"value":"'+strval+'mm"} }')
    else:
      strtowrite=(' "'+strlat+','+strlon+'": {"value":"'+strval+'mm"},')
