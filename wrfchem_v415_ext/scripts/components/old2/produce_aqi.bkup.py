...
listAQI['o3']={
  'boundsugm3':pd.Series([0,34, 67,101,121,141,161,188,214,241], index=np.arange(1,11,1)),
  'hrspan':8,
  'varname':'o3_concentration'
}
listAQI['nox']={
  'boundsugm3':pd.Series([0,68,135,201,268,335,401,468,535,601],index=np.arange(1,11,1)),
  'hrspan':1,
  'varname':'nox_concentration'
}
listAQI['so2']={
  'boundsugm3':pd.Series([0,30,60,90,120,150,180,237,296,355],index=np.arange(1,11,1)),
  'hrspan':1,
  'varname':'so2_concentration'
}
listAQI['pm25']={
  'boundsugm3':pd.Series([0,12,24,36,42,48,54,59,65,71],index=np.arange(1,11,1)),
  'hrspan':24,
  'varname':'pm25'
}
listAQI['pm10']={
  'boundsugm3':pd.Series([0,17,34,51,59,67,76,84,92,101],index=np.arange(1,11,1)),
  'hrspan':24,
  'varname':'pm10'
}
...
#   -----------------------
   ...
   print(tot_aq.shape)
   cs3=plt.contourf(lons, lats, tot_aq, cmap=cm, levels=levels, extend="max", spacing='proportional', ticks=levels)
   #cs3=plt.contourf(lons, lats, tot_aq, cmap=cm, norm=norm, spacing='proportional', ticks='bidbounds', boundaries=bidbounds)
   #cs3=plt.contourf(lons, lats, tot_aq, cmap=cm, norm=norm)
   cs3=cm.set_over('m')
