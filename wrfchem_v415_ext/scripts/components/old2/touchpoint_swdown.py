...
parser.add_argument('-swdown', '--swdown', default = 'swdown', dest = 'swdown', help = 'Name for graup mixing ratio')

parser.add_argument('wrffile', help = 'Path to wrfout or wrfinput file that contains latitude_name, longitude_name,  swdown')
...
outpath = os.path.join(dirpath, '%s-swdown-level-gfs-1.0.json' % (timestr,))
gc=0
outf = open(outpath, 'w')
outf.write('{ ')
print(newf.variables[args.swdown].shape)
for xways in range(0, nx):
  for yways in range(0,ny):
    strlat=str(latit[yways])
    strlon=str(longs[xways])
    rawdatavals=newf.variables[args.swdown][0,yways,xways]
    #print(newf.variables[args.swdown].shape)
    #convdatavals=rawdatavals*1e6
    rounddatavals=int(round(rawdatavals))
