...
parser.add_argument('wrffile', help = 'Path to wrfout or wrfinput file that contains latitude_name, longitude_name, aer_conc')
parser.add_argument('outroot', help = 'Path for output', default = '../public/data/weather/')
parser.add_argument('varname', help = 'varname', default = 'aer')
args = parser.parse_args()
outroot = args.outroot
vname = str(args.varname)
print(vname)



#strex1=("parser.add_argument('-%s_conc', '--%s', default = '%s', dest = '%s', help = '')" % (vname))
uhdr = {}
vhdr = {}
data = [uhdr, vhdr]
...
outpath = os.path.join(dirpath, '%s-aerosol.json' % (timestr,))
gc=0
outf = open(outpath, 'w')
outf.write('{ ')
for xways in range(0, nx):
  for yways in range(0,ny):
    ...
    strtoexec=("rawdatavals=newf.variables['%s'][0,0,yways,xways]" % (vname))
    exec(strtoexec)
    #rounddatavals=round(rawdatavals,0)
    intdatavals=round(rawdatavals,4)
