...
parser.add_argument('-pm10_conc', '--pm10_conc', default = 'pm10', dest = 'pm10_conc', help = 'Name for PM10 conc')

parser.add_argument('wrffile', help = 'Path to wrfout or wrfinput file that contains latitude_name, longitude_name, pm10_conc')
...
outpath = os.path.join(dirpath, '%s-pm10-surface-level-gfs-1.0.json' % (timestr,))
gc=0
outf = open(outpath, 'w')
outf.write('{ ')
for xways in range(0, nx):
  for yways in range(0,ny):
    ...
    rawdatavals=newf.variables[args.pm10_conc][0,0,yways,xways]
    rounddatavals=round(rawdatavals,3)
    strval=str(rounddatavals)
    gc=gc+1
    if gc==gridsize:
      strtowrite=(' "'+strlat+', '+strlon+'": {"value":"'+strval+'ug/m3"} }')
    else:
      strtowrite=(' "'+strlat+','+strlon+'": {"value":"'+strval+'ug/m3"},')
