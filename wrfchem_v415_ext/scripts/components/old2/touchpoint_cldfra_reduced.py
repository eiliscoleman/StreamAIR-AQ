...
parser.add_argument('-cldfra', '--cldfra', default = 'cldfra', dest = 'cldfra', help = 'Name for Cloud Fractional Cover')

parser.add_argument('wrffile', help = 'Path to wrfout or wrfinput file that contains latitude_name, longitude_name,  cldfra')
...
outpath = os.path.join(dirpath, '%s-cldfra-level-gfs-1.0.json' % (timestr,))
gc=0
outf = open(outpath, 'w')
outf.write('{ ')
for xways in range(0, nx):
  for yways in range(0,ny):
    ...
    rawdatavals=newf.variables[args.cldfra][0,0,yways,xways]
    #print(newf.variables[args.cldfra].shape)
    convdatavals=rawdatavals
    rounddatavals=round(convdatavals,2)
