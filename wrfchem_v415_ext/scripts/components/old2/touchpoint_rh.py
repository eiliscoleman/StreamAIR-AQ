...
parser.add_argument('-rh', '--rel_humidity', default = 'rh', dest = 'rel_humidity', help = 'Name for relative humidity')

parser.add_argument('wrffile', help = 'Path to wrfout or wrfinput file that contains latitude_name, longitude_name,  relhumidity')
...
outpath = os.path.join(dirpath, '%s-rh-surface-level-gfs-1.0.json' % (timestr,))
gc=0
outf = open(outpath, 'w')
outf.write('{ ')
for xways in range(0, nx):
  for yways in range(0,ny):
    ...
    rawdatavals=newf.variables[args.rel_humidity][0,yways,xways]
    rounddatavals=round(rawdatavals,1)
    strval=str(rounddatavals)
    gc=gc+1
    if gc==gridsize:
      strtowrite=(' "'+strlat+', '+strlon+'": {"value":"'+strval+'%"} }')
    else:
      strtowrite=(' "'+strlat+','+strlon+'": {"value":"'+strval+'%"},')
