...
dir_op=(basdir+'/data/output-wrf-remapped2')
...
aqi_colours=['cyan', 'mediumseagreen', 'yellow', 'tomato', 'darkred']
listAQI={
  ...
  'desc':pd.Series(['Good', 'Fair', 'Moderate', 'Poor', 'Very Poor'],index=np.arange(1,6,1))
}

listAQI['o3']={
  'boundsugm3':pd.Series([0,80, 120,180,240], index=np.arange(1,6,1)),
  'hrspan':1,
  'varname':'o3_concentration'
}
listAQI['nox']={
  'boundsugm3':pd.Series([0,40,100,200,400],index=np.arange(1,6,1)),
  'hrspan':1,
  'varname':'nox_concentration'
}
listAQI['so2']={
  'boundsugm3':pd.Series( [0,100,200,350,500],index=np.arange(1,6,1)),
  'hrspan':1,
  'varname':'so2_concentration'
}
listAQI['pm25']={
  'boundsugm3':pd.Series([0,10,20,25,50],index=np.arange(1,6,1)),
  'hrspan':24,
  'varname':'pm25'
}
listAQI['pm10']={
  'boundsugm3':pd.Series([0,20,35,50,100],index=np.arange(1,6,1)),
  'hrspan':24,
  'varname':'pm10'
}
...
#   -----------------------
   ...
   lev_range=np.arange(0,6,1)
   levels=lev_range
   bounds=np.linspace(0,6,1)
