...
parser.add_argument('-qgraup', '--qgraup', default = 'qgraup', dest = 'qgraup', help = 'Name for graup mixing ratio')

parser.add_argument('wrffile', help = 'Path to wrfout or wrfinput file that contains latitude_name, longitude_name,  qgraup')
...
outpath = os.path.join(dirpath, '%s-qgraup-level-gfs-1.0.json' % (timestr,))
gc=0
outf = open(outpath, 'w')
outf.write('{ ')
for xways in range(0, nx):
  for yways in range(0,ny):
    ...
    rawdatavals=newf.variables[args.qgraup][0,0,yways,xways]
    #print(newf.variables[args.qgraup].shape)
    convdatavals=rawdatavals*1e6
    rounddatavals=round(convdatavals,3)
