...
parser.add_argument('-qsnow', '--qsnow', default = 'qsnow', dest = 'qsnow', help = 'Name for Snow mixing ratio')

parser.add_argument('wrffile', help = 'Path to wrfout or wrfinput file that contains latitude_name, longitude_name,  qsnow')
...
outpath = os.path.join(dirpath, '%s-qsnow-level-gfs-1.0.json' % (timestr,))
gc=0
outf = open(outpath, 'w')
outf.write('{ ')
for xways in range(0, nx):
  for yways in range(0,ny):
    ...
    rawdatavals=newf.variables[args.qsnow][0,0,yways,xways]
    #print(newf.variables[args.qsnow].shape)
