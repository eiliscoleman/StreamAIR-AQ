...
outpath = os.path.join(dirpath, '%s-wdir-surface-level-gfs-1.0.json' % (timestr,))
gc=0
outf = open(outpath, 'w')
outf.write('{ ')
#normailse the wind components
uu_n=uu10[0,:,:]/w10
vv_n=vv10[0,:,:]/w10
print(uu_n.shape)
#vv10=newf.variables[args.vcomponent]
...
for xways in range(0, nx):
  for yways in range(0,ny):
    ...
    if gc==gridsize:
      ...
      strtowrite=(' "'+strlat+','+strlon+'":"'+strval+'",')
      #strtowrite=(' "'+strlat+', '+strlon+'":"'+strval+',u10here'+uu10[0, yways, xways]+',v10here'+vv10[0,yways,xways]+',')
