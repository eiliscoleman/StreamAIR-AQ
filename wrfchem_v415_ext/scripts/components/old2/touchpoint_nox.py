...
parser.add_argument('-nox', '--nox_conc', default = 'nox_concentration', dest = 'nox_conc', help = 'Name for NOx conc')

parser.add_argument('wrffile', help = 'Path to wrfout or wrfinput file that contains latitude_name, longitude_name, nox_conc')
...
outpath = os.path.join(dirpath, '%s-nox-surface-level-gfs-1.0.json' % (timestr,))
gc=0
outf = open(outpath, 'w')
outf.write('{ ')
for xways in range(0, nx):
  for yways in range(0,ny):
    ...
    rawdatavals=newf.variables[args.nox_conc][0,0,yways,xways]
    rounddatavals=round(rawdatavals,7)*1000.
    strval=str(rounddatavals)
    gc=gc+1
    if gc==gridsize:
      strtowrite=(' "'+strlat+', '+strlon+'": {"value":"'+strval+'ppb"} }')
    else:
      strtowrite=(' "'+strlat+','+strlon+'": {"value":"'+strval+'ppb"},')
