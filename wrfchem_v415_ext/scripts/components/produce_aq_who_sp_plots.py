...
listwho={
  ...
  'bnds':pd.Series(bboundsppb, index=np.arange(0,len(species))),
  'time_ave':pd.Series(time_ave, index=np.arange(0,len(species))),
}
listwho2={
  #'spcs':pd.Series(species, index=np.arange(0,len(species))),
  'bnds':pd.Series(bboundsppb, index=species),
...
#   -----------------------
   ...
   for  spi, sp in enumerate(listwho['spcs']):
     ...
     opspfilename=timestr+'-'+sp+'-ave-who.json'
     ...
     for i, lonv in enumerate(data_ref.lon.values):
         for j, latv in enumerate(data_ref.lat.values):
           ...
           sp_dict[j,i]= spc_ave_conv[j,i]
           
     coords_df =pd.DataFrame.from_dict(coords_dict, orient='index')
     sp_df = pd.DataFrame.from_dict(sp_dict, orient='index')
     sp_df['sp']=sp_df.values
     sp_df = sp_df.round({'sp': 2})
     sp_df['coords'] = coords_df
     text_output = sp_df.set_index('coords').to_dict()['sp']
...
##########################################
   ...
   opspfilename=timestr+'-exc-aq-who.json'
   outpath = os.path.join(dirpath,opspfilename)
   ...
   for i, lonv in enumerate(data_ref.lon.values):
       for j, latv in enumerate(data_ref.lat.values):
           tot_ex_dict[j,i]= tot_ex_mx[j,i]
           
   coords_df =pd.DataFrame.from_dict(coords_dict, orient='index')
   tot_ex_df = pd.DataFrame.from_dict(tot_ex_dict, orient='index')
   tot_ex_df['tot_ex']=tot_ex_df.values
   tot_ex_df = tot_ex_df.round({'tot_ex': 2})
   tot_ex_df['coords'] = coords_df
   text_output = tot_ex_df.set_index('coords').to_dict()['tot_ex']
   with open(outpath, 'w') as fp:
       json.dump(text_output, fp)
