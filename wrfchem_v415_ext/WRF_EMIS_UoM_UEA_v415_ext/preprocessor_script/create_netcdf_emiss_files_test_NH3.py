...
dataset = ds('/mnt/raid/wrf-chem/emis/WRF_EMIS_UoM_UEA/preprocessor_script/OUT/NH3_tno_out.nc','w','r', format='NETCDF4_CLASSIC') #output file
...
#2)Extract other variables (NOx, nh3, etc) using the NETCDF4 library########
tno_net = ds(tno_2011) 
...
nh3_data = tno_net.variables['nh3'][:]  #Array with the emissions per nh3mpound 
...
nh3_pow = np.zeros(shape=(12,672,720))
nh3_res = np.zeros(shape=(12,672,720))
nh3_inc = np.zeros(shape=(12,672,720))
nh3_pei = np.zeros(shape=(12,672,720)) #Since the new inventory have sectors three and four merged as 'sector 34', I have assigned emissions from sector 34 to the category 3 and left category four empty
nh3_exf = np.zeros(shape=(12,672,720))
nh3_sol = np.zeros(shape=(12,672,720))
nh3_tra1 = np.zeros(shape=(12,672,720))
nh3_tra2 = np.zeros(shape=(12,672,720))
nh3_tra3 = np.zeros(shape=(12,672,720))
nh3_tra4 = np.zeros(shape=(12,672,720))
nh3_tra5 = np.zeros(shape=(12,672,720))
nh3_nrt = np.zeros(shape=(12,672,720))
nh3_was = np.zeros(shape=(12,672,720))
nh3_agr = np.zeros(shape=(12,672,720))
...
for i in range(len(lat_index)):
    ...
    if sector == 1:
        for k in range(12):
            nh3_pow[k,lat_index[i]-1, lon_index[i]-1] += nh3_data[i]
    if sector == 2:
        for k in range(12):
            nh3_res[k,lat_index[i]-1, lon_index[i]-1] += nh3_data[i]
    if sector == 3:
        for k in range(12):
            nh3_inc[k,lat_index[i]-1, lon_index[i]-1] += nh3_data[i]
    if sector == 4:
        for k in range(12):
            nh3_exf[k,lat_index[i]-1, lon_index[i]-1] += nh3_data[i] 
    if sector == 5:
        for k in range(12):
            nh3_sol[k,lat_index[i]-1, lon_index[i]-1] += nh3_data[i] 
    if sector == 6:
        for k in range(12):
            nh3_tra1[k,lat_index[i]-1, lon_index[i]-1] += nh3_data[i]        
    if sector == 7:
       for k in range(12):
           nh3_tra2[k,lat_index[i]-1, lon_index[i]-1] += nh3_data[i]  
    if sector == 8:
        for k in range(12):
            nh3_tra3[k,lat_index[i]-1, lon_index[i]-1] += nh3_data[i] 
    if sector == 9:
        for k in range(12):
            nh3_tra4[k,lat_index[i]-1, lon_index[i]-1] += nh3_data[i] 
    if sector == 10:
        for k in range(12):
            nh3_tra5[k,lat_index[i]-1, lon_index[i]-1] += nh3_data[i] 
    if sector == 11:
        for k in range(12):
            nh3_nrt[k,lat_index[i]-1, lon_index[i]-1] += nh3_data[i] 
    if sector == 12:
        for k in range(12):
            nh3_was[k,lat_index[i]-1, lon_index[i]-1] += nh3_data[i]        
    if sector == 13:
        for k in range(12):
            nh3_agr[k,lat_index[i]-1, lon_index[i]-1] += nh3_data[i]        
                        
print (nh3_nrt[5,105,0]) 
print (nh3_nrt.shape) #To check if I am picking the right values 
#print nh3_nrt[5,105,0] 
#print nh3_nrt.shape #To check if I am picking the right values 

####iii)Append values to the variable time (12 months)
#dates = [datetime(2000,01,01)+n*monthdelta(1) for n in range(nh3_pow.shape[0])]     
dates = [datetime(2000,1,1)+n*monthdelta(1) for n in range(nh3_pow.shape[0])]     
...
print ("dates nh3rresponding to time values:\n",dates)
...
#print dataset.file_format # At UEA the TNO emissions preprocessor program nh3mpiles/runs, only, with the netCDF3.6.3 library. I am not able to produce a netcd3 file directly since is an old library. Therefore, I have applied the following nh3nversion
