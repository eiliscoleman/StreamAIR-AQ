...
aer_list=['aermr01','aermr02', 'aermr03', 'aermr04', 'aermr05', 'aermr06', 'aermr07', 'aermr08', 'aermr09', 'aermr10', 'aermr11', 'aermr12', 'aermr13', 'aermr14', 'aermr15', 'aermr16', 'aermr17', 'aermr18']
air_mw=28.97
...
#----------------------------------------------    
# ff(x) = loop to take a sample of the CAMS data rather than the whole global domain. Extend of the reduced domain is define in the domain_red parameter. 
def ff(x):
    ...
    for rawf in sorted(glob.glob(dl_data_path+'/z_cams_c_ecmf_'+datetag+'0000_prod_fc_ml_*_'+str(x)+'.nc')):
        ...
        cdo.sellonlatbox(domain_red, input=rawf, output=box_file)
        # move to a boxed file
        shutil.move(box_file, box_dir_path)
...
#The command under reduces map domain and moves to a specific direcotry 
    ...
    #p.map(ff,spcs_list)
#-------------------------------------------------
# using "no2" files to sample the dataset, obtain the filenames for each time stamp to enable merging of all species into one file per timestep
...
# NOTE: this is all to be taskfarmed out also! First, do with 2 files, then define a function and map out
...
for fs in glob.glob(os.path.join(cams_dir, "*.nc")):
    o_data=Dataset(fs, 'r')
    ...
    lon[:]=o_data.variables['longitude'][:]
    ...
    for ass in aer_list:
        ...
        ##exec("%s[:]=o_data.variables[ss][:,:,:,:]*0.63" % (ss))
        ##no2[:]=o_data.variables['no2'][:,:,:,:]*0.63
