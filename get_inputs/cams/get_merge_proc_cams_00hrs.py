...
iy=dt.datetime.now()-timedelta(days=1)
...
def f(x):
    print(x)
    wget_cmd='wget -m ftp://liz.coleman:fj0yVQEV@dissemination.ecmwf.int:/DATA/CAMS_NREALTIME/'+datetag+'/z_cams_c_ecmf_'+datetag+'0000_prod_fc_ml_*_'+str(x)+'.nc -P op_data/'
    print(wget_cmd)
...
for spp in spcs_list:
    print(spp)
    wget_cmd='wget -m ftp://liz.coleman:fj0yVQEV@dissemination.ecmwf.int:/DATA/CAMS_NREALTIME/'+datetag+'/z_cams_c_ecmf_'+datetag+'0000_prod_fc_ml_*_'+str(spp)+'.nc -P op_data/'
    print(wget_cmd)
    os.system(wget_cmd)
...
def ff(x):
    ...
    for rawf in sorted(glob.glob(dl_data_path+'/z_cams_c_ecmf_'+datetag+'0000_prod_fc_ml_*_'+str(x)+'.nc')):
        ...
        if not os.path.exists(box_dest):
            ...
            sys_cmd=('ncks -d longitude,314.,46. -d latitude,30.,70. '+rawf+' '+ box_dest)
            os.system(sys_cmd)
        else:
...
def q(fs):
    ...
    #New filename  
    dir_tag=('for_mozbc00')
    n_dir=os.path.join(cur_dir, dir_tag)
...
#-------------------------------------------------------     
# 
datetagy=(str(iy.year)+str(iy.month).zfill(2)+str(iy.day).zfill(2)+'00')
print(datetagy)
